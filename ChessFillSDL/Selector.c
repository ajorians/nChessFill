...
{
   struct Selector* pSelector = *ppSelector;
   pSelector->m_Chess = NULL;//Does not own
...
void Move(struct Selector* pSelector, enum Direction eDirection)
{
   switch( eDirection ) {
      ...
      case Up:
         ...
         break;
      case Down:
         if( pSelector->m_nCurrentY < 3 ) pSelector->m_nCurrentY++;
         break;
      case Left:
         if( pSelector->m_nCurrentX > 0 ) pSelector->m_nCurrentX--;
         break;
      case Right:
	 if( pSelector->m_nCurrentX < 3 ) pSelector->m_nCurrentX++;
         break;
   }
}
...
{
   pSelector->m_nCurrentX = 0;
   pSelector->m_nCurrentY = 0;
}
